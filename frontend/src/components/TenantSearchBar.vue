  ...
  <div class="flex">
    <div class="relative w-full ml-2 mr-2">
      <template v-if="showFilters">
        <Select v-model="selectedFilter" :options="filters" :placeholder="$t('tenant_list.select_tag')" />
      </template>
      ...
      <template v-else>
        <input
          ...
          :placeholder="$t('search_bar.search_for')"
          required
        />
      </template>
...
export default defineComponent({
  ...
  watch: {
    selectedFilter: function (val) {
        this.$emit('selectedFilterChanged', val)
    }
  },
  props: {
    filters: {
