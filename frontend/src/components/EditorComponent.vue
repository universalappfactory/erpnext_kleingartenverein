    ...
    <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50">
            ...
            <div class="flex flex-wrap items-center divide-gray-200 sm:divide-x">
                <div class="flex items-center space-x-1 sm:pr-4">
                    <button type="button"
                        ...
                        <i class="fa fa-eye" :class="preview ? 'text-black' : ''" @click="this.preview = !this.preview"
                            aria-hidden="true"></i>
                        <span class="sr-only">Show Preview</span>
                    </button>

                    <Select :items="templates" label="" @itemSelected="(tpl) => this.$emit('templateSelected', tpl)"
                        :placeholder="$t('new_letter.select_template')"></Select>
                </div>
            </div>
        </div>
        <div class="h-full rounded-b-lg">
            ...
            <template v-if="preview">
                <div class="preview-markdown" v-html="renderedContent">
            ...
            <template v-else>
                <textarea id="editor"
                    ...
                    v-model="editorContent" :placeholder="placeholder" required>
                    </textarea>
            </template>
        </div>
    </div>
