    ...
    <div>
        ...
        <div class="overflow-y-scroll">
            <ListComponent @loadMore="tenant.loadMore" :items="tenant.tenants" :checkable="false" :headerList="['Pächter']"
                :hasNext="tenant.pageInfo.hasNext" @show-details="(x: any) => this.showDialog(x.name)">
                <template #item="{ name, plot_link, email_id, mobile_no, customer_group, selected }">
                    <div class="flex p-4">
                        <div class="grid grid-cols-2 content-center">

                            <template v-if="selectable">
                                <Checkbox :selected="selected" @checkChanged="value => this.checkChanged(name, value)" />
                            </template>
                        ...
                        <template v-if="showActions">
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 content-center">
                                ...
                                <template v-if="showEditButton">
                                    <button type="button" @click="(x: any) => this.showDialog(name)"
